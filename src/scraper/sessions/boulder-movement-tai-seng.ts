...
const scrape = async (ctx: Context, slug: string): Promise<Session[]> => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const res = await ctx.http.get(
      "https://widgets.mindbodyonline.com/widgets/schedules/153239/load_markup",
      {
        params: {
    ...
    $(".bw-session").each((index, sessionElem) => {
      ...
      if (!name.includes("Climb Session - Tai Seng")) {
        return;
      }
  ...
  const boulderMovementSessions = sessions.map<BoulderMovementSession>(
    (session) => ({
  ...
  return Promise.all(
    boulderMovementSessions.map(async (boulderMovementSession) =>
      insertOrUpdateSession(ctx.db, {
        ...boulderMovementSession,
        gym_id: gym.id,
      }),
    ),
  );
